//
// Created by Scallions on 2021/4/10.
//
...
void setUpServerHandler(HttpServer &server) {

    server.resource_["^/string/?$"]["POST"] = [](HttpData &request) {
        std::stringstream ss;
        *request.content_ >> ss.rdbuf();
        std::string content=ss.str();
        ss.clear();
        ss << "HTTP/1.1 200 OK\r\nContent-Length: " << content.length() << "\r\n\r\n" << content;
        request.response_ = ss.str();
    };

    server.resource_["^/info/?$"]["GET"] = [](HttpData &request) {
        std::stringstream content_stream;
        content_stream << "<h1>Request:</h1>";
        content_stream << request.method_ << " " << request.path_ << " HTTP/" << request.http_version_ << "<br>";
        for(auto& header: request.headData_) {
    ...
    server.resource_["^/favicon.ico$"]["GET"] = [](HttpData &request) {
        std::stringstream header;
        header << "HTTP/1.1 200 OK\r\n";
        header << "Content-Type: image/png\r\n";
        header << "Content-Length: " << (sizeof favicon) << "\r\n";
        header << "\r\n";
        header << std::string(favicon, favicon + sizeof favicon);
        request.response_ = header.str();

    };

    server.defaultResource_["^/?(.*)$"]["GET"] = [](HttpData& request) {
        std::string filename = "www/";

        std::string path = request.path_match_[1];
